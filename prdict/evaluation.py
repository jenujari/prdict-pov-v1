...
def simulate_single_position(sized_target: pd.Series) -> list[Trade]:
    ...
    exited at the last index (the window-boundary flatten kb/fold_spec.md
    ...
    if direction != 0:
        trades.append(Trade(entry_pos, n - 1, direction, size))
    return trades


def strategy_returns(
    trades: list[Trade], daily_return: pd.Series, entry_bps: float, exit_bps: float
) -> pd.Series:
    ...
    entry day and again on the exit day -- a same-day reversal is two trades,
    so it is charged both an exit leg and an entry leg that day, not one
    discounted round trip (kb/evaluation_spec.md section 3).
    """
    n = len(daily_return)
    ...
    for tr in trades:
        ...
        if tr.entry_pos < tr.exit_pos:
            out[tr.entry_pos : tr.exit_pos] += signed * raw[tr.entry_pos : tr.exit_pos]
...
def baseline_returns(daily_return: pd.Series, entry_bps: float) -> pd.Series:
    """Literal buy-and-hold over the given window: full size, entered once,
    held to the window's end, one-way cost only (kb/evaluation_spec.md
    section 4). `daily_return` must already be sliced to the evaluation window."""
...
def main() -> None:
    ...
    expected = [
        ...
        Trade(7, 7, -1, 0.5),  # entered on the last bar, force-flattened same bar
    ]
    ...
    # -- the same-day double charge on a reversal ---------------------------
    daily_return = pd.Series([0.01, -0.02, 0.03, 0.01, -0.01, 0.02, 0.00, 0.015])
    returns = strategy_returns(trades, daily_return, spec.entry_bps, spec.exit_bps)
    expected_returns = np.array([0.0, 0.0, 0.0087, 0.003, 0.0033, 0.0034, -0.0002, -0.0010])
    ...
    # -- baseline_returns: one-way cost on entry only -----------------------
    window = daily_return.iloc[2:7]
    base = baseline_returns(window, spec.baseline_entry_bps)
    expected_base = np.array([0.03 - 0.0010, 0.01, -0.01, 0.02, 0.00])
    assert np.allclose(base.to_numpy(), expected_base, atol=1e-9), base.tolist()
    print("baseline_returns: single continuous long, one-way cost verified")
    ...
    print("directional_hit_rate: masks neutral origins, matches hand calculation")
