"""Score #39's real TFT predictions through #14's locked trading protocol.

Reads `predictions/tft/{context}_{set}.parquet` (written by
`scripts/train_tft.py`) and runs each context through `prdict/evaluation.py`
...
Identical logic to `scripts/run_xgboost_evaluation.py`, pointed at
`predictions/tft/` instead -- both models emit the same origin-indexed
`y_pred_1..y_pred_10` schema, so the evaluation side needs no per-model
branching.

    uv run python -m scripts.run_tft_evaluation
"""
...
OUT_MD = ROOT / "kb" / "tft_evaluation.md"
...
def main() -> None:
    ...
    lines = [
        ...
        "written before any prediction existed (map decision 8). Identical scoring logic to "
        "`kb/xgboost_evaluation.md` -- both models emit the same prediction schema.",
        "",
